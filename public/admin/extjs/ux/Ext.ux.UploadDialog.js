...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
  ...
  createProgressBar : function() {
    this.progress_bar = this.add(
      new Ext.ProgressBar({
        ...
        anchor: '0',
        value: 0.0,
        text: i18n.progress_waiting_text
      })
    );
  },
  ...
  createGrid : function() {
    ...
    var cm = new Ext.grid.ColumnModel([
      {
        header: i18n.state_col_title,
        width: i18n.state_col_width,
        resizable: false,
        dataIndex: 'state',
        sortable: true,
      ...
      {
        header: i18n.filename_col_title,
        width: i18n.filename_col_width,
        dataIndex: 'filename',
        sortable: true,
        renderer: this.renderFilenameCell.createDelegate(this)
      },
      {
        header: i18n.note_col_title,
        width: i18n.note_col_width, 
        dataIndex: 'note',
        sortable: true,
  ...
  fillToolbar : function() {
    ...
    tb.x_buttons.add = tb.addItem(new Ext.ux.UploadDialog.TBBrowseButton({
      ...
      text: i18n.add_btn_text,
      tooltip: i18n.add_btn_tip,
      iconCls: 'is-add',
...
//    
    tb.x_buttons.remove = tb.addButton({
      text: i18n.remove_btn_text,
      tooltip: i18n.remove_btn_tip,
      iconCls: 'is-delete',
...
//    
    tb.x_buttons.reset = tb.addButton({
      text: i18n.reset_btn_text,
      tooltip: i18n.reset_btn_tip,
      iconCls: 'is-cancel',
...
//    
    tb.x_buttons.upload = tb.addButton({
      text: i18n.upload_btn_start_text,
      tooltip: i18n.upload_btn_start_tip,
      iconCls: 'is-upload',
...
//  
    ...
    tb.x_buttons.close = tb.addButton({
      text: i18n.close_btn_text,
      tooltip: i18n.close_btn_tip,
  ...
  isPermittedFile : function(browse_btn) {
    ...
    if (this.isPermittedFileType(filename)) {
      ...
      Ext.Msg.alert(
        i18n.error_msgbox_title, 
        String.format(
          i18n.err_file_type_not_permitted,
          filename,
          this.permitted_extensions.join(i18n.permitted_extensions_join_str)
        )
      );
      result = false;
    }
  ...
  addFileToUploadQueue : function(browse_btn) {
    ...
    store.add(
      new Ext.ux.UploadDialog.FileRecord({
          ...
          note: i18n.note_queued_to_upload,
  ...
  updateProgressBar : function() {
    if (this.is_uploading) {
      ...
      this.progress_bar.updateProgress(0, i18n.progress_waiting_text);
    }
  },
...
//      );
      tb.x_buttons.upload.setIconClass('is-ux-uploaddialog-uploadstopbtn');
      tb.x_buttons.upload.setText(i18n.upload_btn_stop_text);
      tb.x_buttons.upload.getEl()
        .child(tb.x_buttons.upload.buttonSelector)
        .dom[tb.x_buttons.upload.tooltipType] = i18n.upload_btn_stop_tip;
    }
    else {
...
//        'ext-ux-uploaddialog-indicator-stoped'
//      );
      tb.x_buttons.upload.setIconClass('is-ux-uploaddialog-uploadstartbtn');
      tb.x_buttons.upload.setText(i18n.upload_btn_start_text);
//      tb.x_buttons.upload.getEl()
//        .child(tb.x_buttons.upload.buttonSelector)
//        .dom[tb.x_buttons.upload.tooltipType] = i18n.upload_btn_start_tip;
  ...
  prepareNextUploadTask : function() {
    ...
    record.set('note', i18n.note_processing);
    record.commit();
  ...
  updateRecordState : function(data) {
    if ('success' in data.response && data.response.success) {
      ...
      data.record.set(
        'note', data.response.message || data.response.error || i18n.note_upload_success
      );
    }
    else {
      data.record.set('state', Ext.ux.UploadDialog.FileRecord.STATE_FAILED);
      data.record.set(
        'note', data.response.message || data.response.error || i18n.note_upload_error
      );
    }
  ...
  abortUpload : function() {
    ...
    record.set('note', i18n.note_aborted);
    record.commit();
  },
