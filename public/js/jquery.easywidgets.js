...
(function($){
  ...
  $.fn.EasyWidgets.defaults = {
    ...
    behaviour : {
      ...
      // Miliseconds delay between mousedown and  start
      Delay : 100,

      // Miliseconds delay between mouseup and  stop
      Revert : 100,

      // Determinme the opacity of Widget when start 
      Opacity : 0.8,
  ...
  function PrepareWidgetBehaviour(widget, widgetOnDemand, settings){
    ...
    if(widgetMenu.html() == null){
      ...
      widget.find(s.selectors.editbox).dialog({
    	  autoOpen: false,
    	  height: 300,
    	  width: 350,
    	  modal: true,
    	  buttons: {
    	  	'Zapisz': function() {
    	  		client.edit(_pageId, widget[0].id,$('form',this).serializeArray());
    	  		$(this).dialog('close');
			},
			'Anuluj': function() {
				$(this).dialog('close');
  ...
  function PrepareSortablePlaces(sortableItems, settings){
    ...
    $(s.selectors.places).sortable({
      ...
      delay: s.behaviour.Delay,
      revert: s.behaviour.Revert,
      opacity: s.behaviour.Opacity,
      connectWith: $(s.selectors.places),
      placeholder: s.selectors.placeHolder,
      start : function(e, ui){
        $(ui.helper).addClass('ging');
        return true;
      },
      receive: function(e, ui) {
    	  if($.isFunction(s.callbacks.onRecive)){
              s.callbacks.onRecive(e, ui);
    	  }
      },
      stop : function(e, ui){
        WidgetsPositionsChange(s);
        $(ui.item).css({width : ''});
        $(ui.item).removeClass('ging');
        $(s.selectors.places).sortable('enable');
        if($.isFunction(s.callbacks.onStop)){
          s.callbacks.onStop(e, ui);
        }
        return true;
      }
    });
    
    $(s.selectors.places).draggable('destroy');
    $('.draggable').draggable({
      helper: 'clone',
      containment: 'document',
      handle: s.selectors.header,
      ...
      revert: s.behaviour.Revert,
      opacity: s.behaviour.Opacity,
      connectToSortable: $(s.selectors.places),
      start : function(e, ui){
        $(ui.helper).addClass('ging');
        return true;
      },
  ...
  function AddWidgetEditLink(widget, widgetMenu, settings){
    ...
    if(widget.hasClass(s.options.editable)){
      ...
      $(link).mousedown(function(e){
        ...
        var link = $(this);
        var widget = link.parents(s.selectors.widget);
        var editbox = $('#'+widget[0].id+'-editbox');
        editbox.dialog('open');
